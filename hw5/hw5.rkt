...
(define (racketlist->mupllist r-list)
  (cond [(null? r-list) (aunit)]
        [#t (apair (car r-list)
                   (racketlist->mupllist (cdr r-list))
                   )]
   )
  )
...
(define (mupllist->racketlist m-list)
  (cond [(aunit? m-list) null]
        [#t (cons
             (apair-e1 m-list)
             (mupllist->racketlist (apair-e2 m-list))
             )]
   )
 )
  ...
  (cond [(var? e) 
        ...
        [(mlet? e)
         ...
         [(call? e)
          (letrec ([arg (eval-under-env (call-actual e) env)]
            ...
            (if (closure? clos)
                (eval-under-env (fun-body (closure-fun clos)) (cons (cons inval arg)
                                                                    (closure-env clos)
                                                                    ))
                (error "MUPL can't call a function that isn't a closure"))
            )]
         [(closure? e)
          (closure (append (closure-env e) env)
                   (closure-fun e)
                   )]
         [(apair? e)
        ...
        [(fst? e)
         (let ([v (eval-under-env (fst-e e) env)])
           ...
           (begin
             (print v)
             (error "Can only call fst on a pair"))
           )
           )]
        [(snd? e)
         (let ([v (eval-under-env (snd-e e) env)])
           ...
           (begin
             (print v)
             (error "Can only call snd on a pair"))
           )
           )]
        [(isaunit? e)
...
(define (mlet* lstlst e2)
  (if (null? lstlst)
      ...
      (mlet (car nxt) (cdr nxt) (mlet* rem e2))
        )
      )
  )
